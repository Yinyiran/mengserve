...
alter table product
add ProdStar tinyint(1);
select *
from product desc product create table article (
...
desc banner;
alter table banner
modify BanImg varchar(255) not null;
...
alter table banner
modify BanType int(1) not null;
alter table banner
add SortID int(2) default 99;
update banner
set BanType = 2
where banid in (10009, 10011)
update banner
set BanType = 1
where banid in (10010)
select *
from banner;
insert into banner (BanTargID, BanImg)
values (
    123134,
    "resource/img/2020-06-03/1591175135950.jpg"
  ) create table file (
...
alter table article
add UpdateTime datetime NOT NULL DEFAULT DATETIME ON UPDATE CURRENT_TIMESTAMP;
desc compinfo;
alter table compinfo
add AboutID int(20);
-- Sku --
